...
get_max_weights <- function (pars, forms_X, fam_X, qden, fam_Z, ranges, link, ...) {
  ...
  combsZ <- expand.grid(rep(list(0:1), disZ))
  combs <- expand.grid(rep(list(0:1), disX+disZ))
  ...
  func <- function (x, dis_val=integer(0)) {
    ...
    ZX[1, !disc] <- x
    ZX[1, disc] <- dis_val
    X2 <- ZX[,seq_along(fam_X),drop=FALSE]
    # X <- x[seq_len(fam_X)]
    # Z <- x[-seq_len(fam_X)]
    # beta
    ...
    out <- get_X_density(dat = X2, eta = eta, phi = phi, par2=0*phi,
  ...
  # best <- list(value=func(rep(0.1, dX+length(fam_Z)-sum(disc)),
  #                         rep(0,sum(disc))))
  best <- list(value=Inf)

  n_strts <- 10
  for (i in seq_len(n_strts)) {
    strt <- gen_start_vals(fam_X = fam_X, fam_Z = fam_Z, lrg = rg[[1]], urg = rg[[2]])
    # if (length(ranges) > 0) strt <- runif(length(ranges), min = rg[[1]], max = rg[[2]])
    # else strt <- numeric(0)
  ...
  if (is.infinite(best$value)) stop("No upper bound found")
  # print(-best$value)
  # print(ranges)
  return(-best$value)
...
##' Generate random starting values for function to find maximum of density ratio
##'
##' @param fam families of the relevant variables
##' @param fam_X,fam_Z family vectors for treatment and covariate variables
##' @param lrg,urg vectors of the same length as \code{fam_X} or \code{fam} containing lower/upper bounds
##' @param scale optionally a single number or vector by which to scale the random numbers
##'
gen_start_vals <- function (fam_X, fam_Z, lrg, urg, scale=1) {
  fam_X <- fam_X[fam_X != 5]
  fam_Z <- fam_Z[fam_Z != 5]

  ## categorize other variables
  dubX <- (fam_X == 1) | (fam_X == 2)
  aubX <- (fam_X == 3) | (fam_X == 6)
  # bubX <- (fam_X == ...)

  lrg <- c(rep(-Inf, length(fam_X)), lrg)
  lrg[seq_along(fam_X)][aubX] <- 0
  urg <- c(rep(Inf, length(fam_X)), urg)
  # urg[seq_along(fam_X)][bubX] <- 0

  gen_start_vals2(c(fam_X, fam_Z), lrg=lrg, urg=urg, scale=scale)
}

##' @describeIn gen_start_vals lower level function
gen_start_vals2 <- function (fam, lrg, urg, scale=1) {
  # disc <- (fam == 5)
  out <- numeric(length(fam))

  if (length(scale) == 1) scale <- rep_len(scale, length(fam))
  else if (length(scale) != length(fam)) stop("length of 'scale' should be same as 'fam' or 1")

  binf <- is.infinite(lrg) & is.infinite(urg)
  linf <- is.infinite(lrg) & !is.infinite(urg)
  uinf <- !is.infinite(lrg) & is.infinite(urg)
  ninf <- !is.infinite(lrg) & !is.infinite(urg)
  binf[is.na(binf)] <- FALSE
  linf[is.na(linf)] <- FALSE
  uinf[is.na(uinf)] <- FALSE
  ninf[is.na(ninf)] <- FALSE

  out[binf] <- rnorm(sum(binf))*scale[binf]
  out[linf] <- urg[linf] - rexp(sum(linf))*scale[linf]
  out[uinf] <- lrg[uinf] + rexp(sum(uinf))*scale[uinf]
  out[ninf] <- runif(sum(ninf), min = lrg[ninf], max = urg[ninf])
...
rfrugalParam2 <- function(n, formulas = list(list(z ~ 1), list(x ~ z), list(y ~ x), list( ~ 1)),
  ...
  con = list(max_wt = 1, warn = 1, cop="cop", ZXmethod="mean", trace=0,
             show_weight=FALSE, sim_trace=FALSE)
  ...
  ## get sample Z values
  Z0s <- gen_X_values(n, fam=famZ, pars=pars, LHS=LHS_Z, dX=dZ)$dat
  ...
  if (con$show_weight) cat("Maximum weights: ", M, "\n", sep="")

  ## ready for loop
  OK <- rep(FALSE, n)
  nr <- n

  # if (con$sim_trace) cat(nr, " / ", n, sep="")
  # if (con$sim_trace) printCount(n)
  ...
  while (nr > 0) {
    ...
    nr <- sum(!OK)
    if (con$sim_trace) {
      printCount(nr, prev=onr)
      # cat(paste(paste0(rep("\b", nchar(as.character(onr))+nchar(as.character(n))+3))),
      #     paste0(nr, " / ", n))
    }
  }
