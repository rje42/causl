...
poisson_causl_fam <- function (link) {
  ...
  sim <- function (n, mu) rpois(n, lambda=mu)
...
argument to allow the log-density to be evaluated.  
...
summary(modX)$coefficients
```
So we again see that the estimates are well within two standard errors of the 
true value, and the dispersion parameter is estimated to be 
`r summary(modX)$dispersion`, again close to 1.
