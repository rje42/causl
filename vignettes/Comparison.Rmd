...
glmA1 <- glm(A1 ~ A0*L, family=binomial, data=dat)
w <- predict(glmA1, type="response")
w[dat$A1 == 0] <- 1 - w[dat$A1 == 0]  # transform into propensity score

lmY_A0A1_w <- lm(Y ~ A0*A1, data=dat, weights = 1/w)
summary(lmY_A0A1_w)$coef
```
Notice that the coefficients are now correct.
```{r tab_ipw, echo=FALSE}
tab_ipw <- summary(lm(Y ~ A0*A1, data=dat, weights = 1/w))$coef[,1:2]
...
tab_mle <- cbind(modY$pars$Y$beta[1:4], modY$se$Y$beta[1:4], 
