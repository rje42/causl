...
kableExtra::kbl(results, booktabs=TRUE, format="latex") %>%  ## change to format="latex" for paper
  kableExtra::add_header_above(c(" ","Outcome Regression"=3,"IP Weighting"=3,"Double Robust"=3,"MLE"=3))
kableExtra::kbl(results, booktabs=TRUE, format="latex") %>%  ## change to format="latex" for paper
  kableExtra::add_header_above(c(" ","Outcome Regression"=3,"IP Weighting"=3,"MLE"=3))
# xtable::xtable(results, digits=c(0,rep(c(2,3,3),4)), align="|r|rrr|rrr|rrr|")
mle_results[,-(1:9)]

tab <- tab_ipw[,c(1,3,2)]

tab <- cbind(c(-0.5,0.2,0.3,0), tab, tab[,2]/tab[,3])
tab <- cbind(tab, 2*pnorm(-abs(tab[,5])))
colnames(tab) <- c("truth","estimate","bias","s.e.","z-value","p-value")
rownames(tab) <- c("intercept", "A","B","A cdot B")
kableExtra::kbl(tab, booktabs=TRUE, format="latex", digits=c(1,3,3,3,2,2))

# %>%  ## change to format="latex" for paper
#   kableExtra::add_header_above(c(" ","MLE"=3))

```


The code below (which we do not run because it takes over a minute) will allow 
you to compare $N=10^3$ results each with sample size $n=10^3$ via a naïve 
...
```{r one_big_sim, eval=FALSE, echo=FALSE}
set.seed(123)
n <- 1e4
dat <- causalSamp(n, formulas = forms, pars = pars, family = list(3,c(5,5),1,1))
...
## get IPW estimates
lm_ipw <- summary(svyglm(Y ~ A0*A1, design=svydesign(~1, weights=wts, data=dat)))
lm_or <- summary(svyglm(Y ~ A0*A1, design=svydesign(~1, weights=~1, data=dat)))

res <- tibble(truth=c(-1,0.5,0.5,0), estimate=lm_ipw$coefficients[,1], se=lm_ipw$coefficients[,2])
res <- res %>% mutate(bias=estimate-truth, .after=2) %>%
  mutate(z=bias/se, pval=2*pnorm(abs(bias/se), lower.tail = FALSE))
res
res_w <- tibble(truth=c(-1,0.5,0.5,0), estimate=lm_or$coefficients[,1], se=lm_or$coefficients[,2])
res_w <- res_w %>% mutate(bias=estimate-truth, .after=2) %>%
  mutate(z=bias/se, pval=2*pnorm(abs(bias/se), lower.tail = FALSE))
res_w


library(kableExtra)
kableExtra::kbl(res, "latex", booktabs = TRUE, digits = c(1,3,3,3,2,2), 
                col.names = c("truth", "estimate", "bias", "s.e.", "z-value", "p-value")) %>%
  column_spec(3, background="red!30", bold=TRUE)
kableExtra::kbl(res_w, "latex", booktabs = TRUE, digits = c(1,3,3,3,2,2), 
                col.names = c("truth", "estimate", "bias", "s.e.", "z-value", "p-value")) %>%
  column_spec(3, background="red!30")
