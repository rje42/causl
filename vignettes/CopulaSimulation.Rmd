...
pairs(dat)
```
Note that the distribution is as we expect.  Regressing `x` on `z`
...
Regressing `y` on a weighted `x` gives something which is slightly _further away_
than the naïve estimator:
```{r checkY}
library(survey)
dsgn <- svydesign(~1, data=dat, weights = 1/dnorm(predict(lmXZ)))
lmYXn <- lm(y ~ x, data=dat)
lmYX <- svyglm(y ~ x, design = dsgn)
summary(lmYXn)$coefficients
summary(lmYX)$coefficients
```
