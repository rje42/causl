...
This vignette gives an overview on how to use copulas to simulate from a causal 
model using the `causl` package.
...
We can fit a model using maximum likelihood.  Since computing the sandwich standard
errors is slow we turn this option off, but one can easily change this.
```{r fit_model}
out <- fit_causl(dat, formulas = list(z~1, y~x, ~x), control = list(sandwich=FALSE))
out
```
...
```{r fit_other}
out2 <- fit_causl(dat, form = list(y~x, z~x, ~x), control = list(sandwich=FALSE))
out2
```
