...
These coefficients look very close to the true values of `r pars$Y$beta[1]` and
`r pars$Y$beta[2]`.  We can also compare with a naïve method that ignores 
reweighting, giving a bias that is clearly too high.
...
```
We do not run this, because computation with the Frank copula is slow.  The code 
below can be used to estimate the causal effects.
```{r chkAY5, echo=FALSE, eval=FALSE}
...
```
This time the estimates for the intercept and slope are 3203 (8) and $-500$ 
(13) respectively.  
<!-- ` round(est5[1,1])` (` round(est5[1,2],1)`) and a  -->
<!-- causal effect of ` round(est5[2,1],1)` (` round(est5[2,2],1)`).   -->
